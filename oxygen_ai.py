def main():
    ...
    conn = create_engine(f"postgresql://{pg_user}:{pg_password}@{pg_host}/{pg_dbname}")
    ...
    def derive_sentiment(model_output: {"label": str, "score": float}) -> float:
        ...
        return model_output["score"] * (
            1 if model_output["label"] == "yes_fire" else -1
        )

    def predict_fire_from_temp(
    ...
    def predict_fire(
        ...
        if not tweets:
            ...
            flat_temperatures = [
                temp for temp_batch in temperatures for temp in temp_batch
            ]
        ...
        for i in range(len(temperatures)):
            ...
            average_tweet_sentiment = average_tweet_sentiment / len(
                tweet_batch_result
            )
            ...
            if not average_tweet_sentiment:
                ...
                output.append(
                    average_tweet_sentiment * 0.4 + temperature_sentiment * 0.6
                )
    ...
    def parse_key(s: str):
        day_str, coordinates_str = s.split("(")
        day = int(day_str)
    ...
    async def ai_model(batched_ids, data):
        memphis = Memphis()
        ...
        try:
            ...
            producer = await memphis.producer(
                station_name=f"{station_name}", producer_name=f"{station_name}-producer"
            )
            for i in range(len(batched_ids)):
                print(f"Processing batch {i + 1} of {len(batched_ids)}")
                batch = batched_ids[i]

                input_samples = data[data["ID"].isin(batch)]
                input_tweets = input_samples["tweets"].tolist()
                input_temperatures = input_samples["temperature"].tolist()

                predictions = predict_fire(input_tweets, input_temperatures)
                msgs = []
                highest_score = {"score": -1, "day": 0, "x": -1, "y": -1}
                lowest_score = {"score": 1, "day": 0, "x": -1, "y": -1}
                ...
                for j in range(len(predictions)):
                    ...
                    if prediction > 0.7:
                        ...
                        msg["score"] = prediction
                        msgs.append(msg)

                    if prediction > highest_score["score"]:
                        highest_score["score"] = predictions[j]
                        highest_score["day"] = day
                        highest_score["x"] = x
                        highest_score["y"] = y
                        highest_score["j"] = j
                    elif prediction < lowest_score["score"]:
                ...
                print(
                    f"Highest score: {highest_score['score']} on day {highest_score['day']} at ({highest_score['x']}, {highest_score['y']})"
                )
                print(
                    f"Lowest score: {lowest_score['score']} on day {lowest_score['day']} at ({lowest_score['x']}, {lowest_score['y']})"
                )
        except (
            MemphisError,
            MemphisConnectError,
    ...
    def postgres_egress(msg_list):
        ...
        for msg in msg_list:
            ...
            value_list.append(
                {"id": id, "day": day, "xy": func.point(x, y), "score": score}
            )
    ...
    ai_firealerts_production = Table(
        "ai_fire_alerts_production",
        metadata,
    ...
    temp_readings = pd.read_sql_query(
        text("SELECT * FROM public.temp_readings_production"), conn
    )
    ...
    for name, group in tqdm(temp_readings.groupby("xy")):
        ...
        for temp, i in temps:
            samples.append({"day": i, "xy": name, "temp": temp})
    ...
    print("Merging data")
    merged_data = (
        pd.merge(samples, tweets, how="left", on=["xy", "day"])
        .drop(["id", "score"], axis=1)
        .fillna("")
    )

    complete_samples = (
        merged_data.groupby(["xy", "day"])
        .agg(
    ...
    complete_samples[["x", "y"]] = (
        complete_samples["xy"].str.strip("()").str.split(",", expand=True).astype(int)
    )
    complete_samples = complete_samples.drop("xy", axis=1)
    complete_samples["ID"] = (
        complete_samples["day"].astype(str)
        + "("
        + complete_samples["x"].astype(str)
        + ","
        + complete_samples["y"].astype(str)
        + ")"
    )
    complete_samples = complete_samples.sort_values(["day", "x", "y"])

    # Define batch size
    batch_size = 128

    ids = complete_samples["ID"].values

    batches = [ids[i : i + batch_size] for i in range(0, len(ids), batch_size)]

    # Call predict_fire on each batch and map the outputs to the keys
    asyncio.run(ai_model(batches, complete_samples))
